...
*/
   ...
   System.out.println(stmt + " <-- stmt(functionEmpList)");
...
<html>
   ...
   <%
 	...
 	%>
	 		<a href="<%=request.getContextPath()%>/functionEmpList.jsp?currentPage=<%=minPage - pagePerPage%>">이전</a> <!-- ex) minPage가 11이면 11 - 10: 1페이지로 이동 -->
    <% 
		 }
    ...
    %>
	   		<a href="<%=request.getContextPath()%>/functionEmpList.jsp?currentPage=<%=minPage + pagePerPage%>">다음</a> <!-- ex) minPage가 1이면 1 + 10: 11페이지로 이동  -->
	<%
    	}
	%>
	</body>
