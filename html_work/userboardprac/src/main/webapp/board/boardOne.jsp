...
<%
	...
	// 1. 컨트롤러 계층
	// home.jsp에서 boardNo값 넘어오지 않았을 경우 home.jsp로 다시 이동
...
<html>
	...
	<body>
		...
		<table class="table table-bordered">
			<tr>
				<th class="table-warning text-center">게시글 번호</th>
				<td><%=board.getBoardNo()%></td>
			</tr>
			<tr>
				<th class="table-warning text-center">카테고리명</th>
				<td><%=board.getLocalName()%></td>
			</tr>
			<tr>
				<th class="table-warning text-center">게시글 제목</th>
				<td><%=board.getBoardTitle()%></td>
			</tr>
			<tr>
				<th class="table-warning text-center">게시글 내용</th>
				<td><%=board.getBoardContent()%></td>
			</tr>
			<tr>
				<th class="table-warning text-center">작성자</th>
				<td><%=board.getMemberID()%></td>
			</tr>
			<tr>
				<th class="table-warning text-center">작성일자</th>
				<td><%=board.getCreatedate().substring(0, 10)%></td>
			</tr>
			<tr>
				<th class="table-warning text-center">수정일자</th>
				<td><%=board.getUpdatedate().substring(0, 10)%></td>
			</tr>
		</table>
		<br>
		
		<!-- comment(댓글 입력) : 세션유무에 따른 분기 -->
		<!-- 세션 확인하여 로그인한 사용자만 댓글 입력 허용 -->
		...
		<%
			if (session.getAttribute("loginMemberID") != null) { // 세션이 존재하는 경우 (로그인한 경우)
				String loginMemberID = (String) session.getAttribute("loginMemberID");
		%>
				<!-- 로그인 -->
				<form action="<%=request.getContextPath()%>/board/insertCommentAction.jsp" method="post">
					<input type="hidden" name="boardNo" value="<%=board.getBoardNo()%>"> <!-- 출력값 보이지 않음  -->	
					<input type="text" name="memberID" value="<%=loginMemberID%>" readonly="readonly">
					<table class="table table-bordered mx-auto">
						<tr>
							<th class="table-warning text-center">댓글 내용</th>
							<td>
					...
					<button type="submit" class="btn btn-outline-warning">댓글 입력</button>
				</form>
				<br>
		<% 
			}
		...
		%>
			<tr class="text-center">
				...
				<td><a href="" class="btn btn-outline-warning">수정</a></td>
				<td><a href="" class="btn btn-outline-warning">삭제</a></td>
			</tr>
		<% 
			}
		%>
...
<!-- 			<div>
				...
				<a href="" class="btn btn-outline-warning">삭제</a>
			</div> -->
		<%
			}
		%>
		...
		%>
				<a href="<%=request.getContextPath()%>/board/boardOne.jsp?boardNo=<%=boardNo - 1%>" class="btn btn-outline-warning">이전 글</a>
		<%
			}
		%>
		...
		%>
				<a href="<%=request.getContextPath()%>/board/boardOne.jsp?boardNo=<%=boardNo + 1%>" class="btn btn-outline-warning">다음 글</a>
		<%
			}
		%>
		</div>
