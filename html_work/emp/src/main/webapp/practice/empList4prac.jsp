...
<%
	...
	// 초기 파라미터값 확인
	System.out.println(request.getParameter("currentPage") + " <-- currentPage_current param(empList4prac)");
	System.out.println(request.getParameter("gender") + " <-- gender_current param(empList4prac)");
	System.out.println(request.getParameter("searchWord") + " <-- searchWord_current param(empList4prac)");
	...
	System.out.println(currentPage + " <-- currentPage(empList4prac)");
	...
	// 유효값 디버깅
	System.out.println(currentPage + " <-- currentPage(empList4prac)");
	System.out.println(gender + " <-- gender(empList4prac)");
	System.out.println(searchWord + " <-- searchWord(empList4prac)");
	...
	System.out.println("드라이버 로딩 성공(epmList4prac)");
	
	Connection conn = DriverManager.getConnection(dbUrl, dbUser, dbPw);
	System.out.println("DB 접속 성공(empList4prac)");
	
	// Model Layer
	// 쿼리 분기
	// 1) 둘 다 공백 2) searchWord값 공백 3) gender값 공백 4) 둘 다 유효값
	...
	System.out.println(stmt + " <-- stmt(empList4prac)");
	
	ResultSet rs = stmt.executeQuery();
	System.out.println(rs + " <-- rs(empList4prac)");
	...
	System.out.println(totalRow + " <-- totalRow(empList4prac)");

	int lastPage = totalRow / rowPerPage;
	if (totalRow % rowPerPage != 0) {
		lastPage = lastPage + 1;
	}
	System.out.println(lastPage + " <-- lastPage(empList4prac)");
...
<html>
	...
	<body>
		...
		<form action="./empList4prac.jsp" method="get">
	 	<label>성별: </label>
			<select name="gender">
			<%
				if (gender.equals("")) {
		...
		<% 
			if (currentPage > 1) { 
		%>
		 	<a href="./empList4prac.jsp?currentPage=<%=currentPage - 1%>&gender=<%=gender%>&searchWord=<%=searchWord%>" class="btn btn-primary">이전</a>
		<% 
			}
		%>
			<%=currentPage%>페이지
		...
		%>
			<a href="./empList4prac.jsp?currentPage=<%=currentPage + 1%>&gender=<%=gender%>&searchWord=<%=searchWord%>" class="btn btn-primary">다음</a>
		<%
			}
		%>
		</div>
